from Rematch_AHP_helper import get_ranks
import helper
import sys
import copy
...
def node_map(substrate, virtual, req_no):
    ...
    sorder = sorted([i for i in range(substrate.nodes)], key=lambda x: substrate.node_weights[x], reverse=True) # ascending order
    ...
    for vnode in vorder:
        for snode in sorder:
            if substrate.node_weights[snode] >= virtual.node_weights[vnode] and snode not in assigned_nodes:
                map[vnode] = snode
                logging.info(f"\t\tVirtual Node {map[vnode]} is Mapped on --> Substrate Node-{snode}")
...
def main():
    ...
    logging.basicConfig(filename="Rematch.log",filemode="w", level=logging.INFO)
    ...
    pre_resource_nodecost = sum(substrate.node_weights.values()) # total of the physical network
    ...
    # print(f"Dynamic : VNRs to be re-embedded : {vnrs_for_dynamic}")
    ...
    # Generate three additional VNEs
    additional_vne = []
    ...
    i = 0
    for vnr_number in vnrs_for_dynamic : 
        vne_list[vnr_number] = copy.deepcopy(additional_vne[i])
        i += 1
    ...
    for req_no in vnrs_for_dynamic :
        ...
        if req_map is None:
            # print(f"Dynamic Re-embedding : Node mapping not possible for req no {req_no}")
            logging.warning(f"\tNode mapping not possible for req no {req_no}\n")
            continue
        # print(f"Dynamic Re-embedding : Node mapping success for req no {req_no}")
        req_map = temp_map(vne_list,req_no,req_map)
        if not edge_map(substrate, virtual_request, req_no, req_map,vne_list):
            # print(f"Dynamic Re-embedding : Edge mapping not possible for req no {req_no}")
            logging.warning(f"\tEdge mapping not possible for req no {req_no}\n")
            continue
        # print(f"Dynamic Re-embedding : Edge mapping success for req no {req_no}")
        dynamic_accepted += 1
        path_count_modified = 0
        for ed in req_map.edge_map:
            path_cnt += len(req_map.edge_map[ed])
        ...
        revenue += sum(vne_list[req_no].node_weights.values()) + sum(vne_list[req_no].edge_weights.values())//2
        logging.info(f"\t\tMapping for request {req_no} is done successfully!! {req_map.node_map} with revenue {sum(vne_list[req_no].node_weights.values()) + sum(vne_list[req_no].edge_weights.values())//2} and total cost {req_map.total_cost}\n")
    total_accepted = accepted + dynamic_accepted
    ...
    if (total_accepted != 0):
        ...
        avg_path_length_modified /= total_accepted
    ...
    config.curr_map = pickle.loads(pickle.dumps(curr_map,-1))
    ...
    config.active_servers = pickle.loads(pickle.dumps(utilized_servers,-1))
    if tot_cost == 0:
