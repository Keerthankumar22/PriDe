...
class NetworkAttribute(object):
  ...
  def __init__(self, network, virtual=False, **kwargs):
    ...
    self.node_degree = {}
    self.security_probability = kwargs.get('security_probability', {})
  ...
  def generate_network_security_probability(self, original_net):
    if self.security_probability:
      return self.security_probability
    network_security_probability = dict()
    for i in self.network:
      network_security_probability[i] = original_net.security_probability[i]
    #print ("Initial Node CRB %s" % network_crb)
    self.security_probability = network_security_probability
    print(network_security_probability, "generated")
    return network_security_probability

  def get_network_security_probability(self):
    return self.security_probability
  ...
  def normalized_security_probability(self, original_net):
    nt_security_probability = self.generate_network_security_probability(original_net)
    total_n_value = math.sqrt(sum(value ** 2 for value in nt_security_probability.values()))
    normalised_security_probability = {}
    for elem in nt_security_probability:
      normalised_security_probability[elem] = float(nt_security_probability[elem])/float(total_n_value)
    _normalised_security_probability = {ky:round(vl, 2) for ky, vl in normalised_security_probability.items()}
    #print( 'Normalised CRB : %s' % _normalised_crb)
    return _normalised_security_probability
  ...
  def normalized_node_degree(self):
    ...
    total_n_value = math.sqrt(sum(value ** 2 for value in nt_degree.values()))
    normalised_degree = {}
