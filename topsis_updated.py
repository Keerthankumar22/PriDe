...
from topsis_helper_new import get_ranks #without katz
...
logging.basicConfig(filename=r'topsis.log',
                    format=FORMAT,
                    filemode="w",
                    level=logging.DEBUG,
...
def main():
    ...
    req_order = sorted(list(range(len(vne_list))), key=lambda x:revenue_list[x]) # Asceding order of revenue
    ...
    i = 0
    for vnr_number in vnrs_for_dynamic:
        vne_list[vnr_number] = pickle.loads(pickle.dumps(additional_vne[i],-1))
        i += 1
    ...
    config.active_servers = pickle.loads(pickle.dumps(utilized_servers,-1)) # saves the servers 
