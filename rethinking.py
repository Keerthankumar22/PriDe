...
def edge_mapping(substrate,vne_list,req_no,req_map,population) :
    initial_population = []
    population_set = set()
    for i in population:
        abhi_map = i
...
def main():
    ...
    for req_no in req_order:
        ...
        if population is None or len(population) == 0:
            ...
            continue
        selected_map = edge_mapping(substrate,vne_list,req_no,req_map,population)
        logging.info(f"\n")
        ls = dict()
        c =0
    ...
    for vnr_number in vnrs_for_dynamic :
        if vnr_number in curr_map :
            ...
            for i, node in enumerate(mapping_for_the_vne.node_map):
                substrate.node_weights[node] += virtual.node_weights[i]
            # Release bandwidth 
            for i, path in enumerate(mapping_for_the_vne.edge_map):
                for j in range(1, len(path)):
                    substrate.edge_weights[
                        (str(path[j - 1]), str(path[j]))
                    ] += mapping_for_the_vne.edge_weight[i]
                    substrate.edge_weights[
    ...
    for req_no in vnrs_for_dynamic :
        ...
        curr_map[req_no] = selected_map
        revenue += sum(vne_list[req_no].node_weights.values()) + (sum(vne_list[req_no].edge_weights.values())//2)
        substract_from_substrate(substrate,virtual_request,selected_map)
