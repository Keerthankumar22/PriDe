...
class WeightMatrix(object):
  def __init__(self, network, crb, bandwidth, betweenness, eigen, degree, security):
  ...
  def generate_katz(self):
    ...
    self.katz = nx.katz_centrality(g, max_iter=1000, tol=1.0e-6, normalized=True)
    return self.katz
  ...
  def get_weight_matrix(self):
    ...
    for idx, _node in enumerate(self.vertices):
      ...
      tp_matrix[idx].append(self.eigen[idx])     #if using using inbuilt eigen vector function
      tp_matrix[idx].append(self.degree[_node])
      tp_matrix[idx].append(self.security[_node])
  ...
  def compute_performance_indices_matrix(self):
    ...
    sum_crb, sum_bdwth, sum_bc, sum_eig, sum_dgr, sum_security = 0, 0, 0, 0, 0, 0
    for row in matrix:
      sum_crb += row[0]
      sum_bdwth += row[1]
      sum_bc += row[2]
    ...
    for idx, _row in enumerate(matrix):
      ...
      nor_matrix[idx][5] = round(float(_row[5]) / float(sum_security), 3)
  ...
  def compute_entropy_measure_matrix(self):
    ...
    sum_matrix = [0 for _ in range(6)]
    ent_mx_stp1 = [[0, 0, 0, 0, 0, 0] for i in range(len(self.network))]
    for idx, _row in enumerate(perf_matrix):
      ...
      sum_matrix[4] += n_degree
      security = self.compute_nlog(_row[5])
      ent_mx_stp1[idx][5] = security
      sum_matrix[5] += security
    ...
    logging.info(f"\t\t NORD The Weight_mx is sum_crb, sum_bdwth, sum_bc, sum_eig, sum_dgr, sum_security {weight_mx}%")
# TOPSIS will start here
    ...
    for i in range(len(self._perf_mx)):
      for k in range(len(weight_mx)):
        weighted_nor_mx[i][k] = self._perf_mx[i][k] * weight_mx[k] # NORMALIZED MATRIX * wEIGHTS
    ...
    node_rank = {key: rank for rank, key in enumerate(sorted(rank_dict,key=rank_dict.get, reverse=True), 1)}
    # print ('+' * 100 + '\n')
