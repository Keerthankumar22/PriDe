...
class Graph:
    def __init__(self, nodes, edges, parameters) -> None:
        ...
        self.rsa_embedding = dict() # store list of RSA values of VMs mapped on a server
        ...
        for i in range(self.nodes):
            ...
            self.security_probability[i] = np.random.uniform(lower_security_prob, upper_security_prob)
            self.rsa_values[i] = np.random.randint(lower_rsa, upper_rsa)
            self.rsa_embedding[i] = dict() # dictionary to store the rsa values from VMs
            l = list()
