...
def node_map(substrate, virtual, req_no):
    ...
    sorder = get_ranks(substrate) # desendingcending order
    logging.info(f"\n\n\t\t\t\t\t\tSUBSTRATE NETWORK AFTER Ranking {sorder}")
    vorder = get_ranks(virtual)
    logging.info(f"\n\n\t\t\t\t\t\t  {req_no} AFTER Ranking {vorder}")
    assigned_nodes = set()
    for vnode in vorder:
        for snode in sorder:
...
def main():
    ...
    logging.info(f"\n\n\t\t\t\t\t\tNRM-Started")
    logging.info(f"\n\n\t\t\t\t\t\tNRM-Started SUBSTRATE NETWORK (BEFORE MAPPING VNRs)")
    ...
    for req_no in vnrs_for_dynamic :
        ...
        revenue += sum(vne_list[req_no].node_weights.values()) + sum(vne_list[req_no].edge_weights.values())//2
        logging.info(f"\t\tMapping for request {req_no} is done successfully!! {req_map.node_map} with revenue {sum(vne_list[req_no].node_weights.values()) + sum(vne_list[req_no].edge_weights.values())//2} and total cost {req_map.total_cost}\n")
    total_accepted = dynamic_accepted + accepted
    ed_cost  = 0
    no_cost = 0
