...
def get_ranks(graph):
    ...
    option_mat = np.array([[graph.node_weights[i] for i in range(graph.nodes)], [graph.node_weights[i] for i in range(graph.nodes)]])
    ...
    ranking = sorted([i for i in range(graph.nodes)], key=lambda x: option_mat[x])
    return ranking
